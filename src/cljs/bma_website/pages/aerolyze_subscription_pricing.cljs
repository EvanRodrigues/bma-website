(ns bma-website.pages.aerolyze-subscription-pricing
  (:require [bma-website.parts.misc :as misc]
            [bma-website.parts.pricing-calculator :as pricing-calculator]))
...
(defn html []
  [:div {:class "codrops-bodyBMA"}
   (misc/banner-and-title "./images/site/logos_and_headers/BMA_WEBSITE_AEROLYZE_LOGO_HEADER_narrow.jpg" "aerolyze logo header" "Aerolyze As A Subscription")
   ...
   [:div {:class "content clearfix"}
    ...
    [:br]
    [:h2 {:style {:color "#316bad" :text-align "center" :text-decoration "underline"}} "Transaction & Module Cost"]
    ...
    [:h3 [:br] "Read on to determine which one of the transaction levels is right for your business."]
    ...
    [:div {:class "blockBMA blockBMA-40 blockFloat clearfix"}
     ...
     [:h2 "Full Moon"]
     [:h2 {:style {:color "#000033"}} "$4,525 per Month " [:br] [:i "Billed Annually"]]
     [:h3 {:style {:color "#316bad"}} "Analyze Up To 150 Opportunities"]
    ...
    [:div {:class "codrops-bodyBMA clearfix"}
     ...
     [:h3 "Transaction level service refers to the quantity of opportunities that your organization can analyze each year. We view an opportunity as a single deal, which could include a stand alone aircraft or up to a total of 30 aircraft. Each opportunity will be accessible for three (3) months from the created date. Your data will be backed up and never deleted."] 
     [:h3 "Each of the transaction and module costs for the base app and additional modules are billed annually per company by default."]
     [:h4 [:i "Electing to pay month-to-month will incur a 15% surcharge added to the total costs outlined on this page."]]
     ]]
   
   ;; User Pricing
   [:div {:class "content clearfix"}
    [:h2 {:style {:color "#316bad" :text-align "center" :text-decoration "underline"}} "User License Cost"]
    ...
    [:h3 "Onboarding refers to initial data migration into Aerolyze, as well as our insistent one-on-one introduction and Q&A sessions to get first time users familiar with the software. Contact us for further details."]
    
    [:h3 "Both types of licenses are billed annually per license. Team-level licenses are required with each transaction level subscription. Manager-level licenses are elective."]
    [:h3 [:b "Included with each annual subscription is our high-touch customer service. In addition to one-on-one initial on-boarding sessions, we are available within 24 hours to assist with Aerolyze. If you get stuck or need advice on how to address a pricing problem from within Aerolyze we will do our best to assist."]]
    ...
    [:div {:class "codrops-bodyBMA clearfix" :style {:margin "0 auto"}}
     [:h2 {:style {:color "#316bad" :text-align "center" :text-decoration "underline"}} "Aerolyze Pricing Tool"]
